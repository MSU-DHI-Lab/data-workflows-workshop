...
Goal: start NiFi in Docker, load the UI, and get comfortable with processors and queues.

Inputs: `docker-compose.yml` (in this lab folder)
...
2) **Do:** Run `docker-compose up` (or `docker compose up`) in this directory.  
   ...
   **If it doesn't look right:** Retry with `docker compose up` if your Docker uses the newer syntax; check that port 8080 is free.
...
3) **Do:** Visit `http://localhost:8080/nifi/` in your browser.  
   ...
   **If it doesn't look right:** Accept the local certificate warning (expected in local dev); confirm the container is still running.
