...
1) **NiFi won't start in Docker**  
   - Fix: Check Docker is running; ensure address 8080 is free; retry `docker compose up`.

2) **Cannot reach UI / certificate warning**  
   - Fix: Accept the local cert warning (expected); confirm NiFi is healthy in Docker Desktop.
...
7) **PutFile fails to write**  
   - Fix: Ensure mounted directories exist; check Docker logs for permission errors.

8) **Run record empty**  
   - Fix: Expand the time window; ensure processors are running; look at queued FlowFiles and use View provenance (NiFi's run record view).
