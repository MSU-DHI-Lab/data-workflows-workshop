...
2) **Validation rules report missing columns**  
   - Fix: Ensure column names match exactly; adjust the rules or rename columns in the data.
...
5) **Validation stops on first error**  
   - Fix: Use `lazy=True` when running validation to collect all failures.
