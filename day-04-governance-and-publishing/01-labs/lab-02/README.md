...
2) **Do:** Update `artifacts/CITATION.cff` with title, authors, version, and date.  
   ...
   **You should see:** YAML with your details, `cff-version: 1.2.0` intact.  
...
3) **Do:** Fill `artifacts/.zenodo.json` with your metadata (title, description, creators, keywords, license, version).  
   ...
   **You should see:** Valid JSON with your entries.  
   **If it doesn't look right:** Validate JSON; ensure required fields are present.
...
4) **Do:** Walk through `publishing_checklist.md` and tick completed items.  
   ...
   **If it doesn't look right:** Return to Lab 01 to fill gaps (README, dictionary, provenance, checks).
