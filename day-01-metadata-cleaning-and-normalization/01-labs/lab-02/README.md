# Lab 02: Clean, Normalize, and Export a Reproducible Recipe

Goal: apply targeted fixes to rights, place, and creator fields, then export both the cleaned CSV and the operations JSON recipe.
...
- Operations JSON saved to `artifacts/openrefine_operations.json` (already provided; update with your own if you add steps)
- Data dictionary and dataset README templates in `artifacts/`

Time: ~30 minutes.

## Why these fields and why "just enough" normalization
- **Rights:** high-impact for reuse; ambiguity blocks sharing. We normalize to a short allowed list without inventing new statements.
- **Place:** common search facet; small differences break aggregation. We standardize city/state while keeping the original intent.
...
- The operations JSON is the most important deliverable because it lets anyone replay your exact steps on future exports.
...
2) **Do:** Apply the provided operations JSON (`artifacts/openrefine_operations.json`) via Undo/Redo → Apply.  
   ...
   **If it doesn't look right:** Ensure you selected the JSON file, not the CSV; check the Undo/Redo tab for errors and reapply.

3) **Do:** Add a text facet on `rights` to confirm only the allowed tokens remain.  
   **Why:** Quick validation that the rights normalization stuck.  
   **You should see:** Only `CC BY 4.0`, `Public Domain`, `Rights Reserved`.  
   **If it doesn't look right:** Reapply the operations JSON; check for trailing spaces and run a trim transform on `rights` if needed.

4) **Do:** Add a text facet on `place` to confirm variants collapsed.  
   **Why:** Ensures place normalization did not leave stray variants.  
   **You should see:** `New York City, NY` and `Albany, NY`.  
   **If it doesn't look right:** Look for capitalization differences; reapply the place transforms from the JSON or adjust the expression and re-export the recipe.
...
7) **Do:** Export your updated operations JSON (Undo/Redo → Extract → Save to `artifacts/openrefine_operations.json`).  
   **Why:** Captures the exact steps for reruns. This is the scaling move for future data pulls.  
   **You should see:** A JSON file downloaded; it should list your transforms.  
...
8) **Do:** Open `artifacts/data_dictionary_template.md` and `artifacts/dataset_readme_template.md`; fill them with your cleaned field definitions and notes, then save into `05-artifacts/` if desired.  
   ...
   **You should see:** Completed files with every field described, rights guidance noted, and provenance filled in, saved alongside your cleaned CSV.  
